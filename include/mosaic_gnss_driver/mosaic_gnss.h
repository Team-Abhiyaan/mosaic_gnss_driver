...
namespace mosaic_gnss_driver {
    ...
    public:
         ...
         */
        explicit GNSS(mosaic_gnss_driver::DataBuffers& buffers) :
            data_buf{buffers}, conn{buffer}, p{data_buf}
        {
            ...
            // static_assert("TODO: Check that parser has the `parse(const uint8_t *data, size_t
            // size)` method")
        };
