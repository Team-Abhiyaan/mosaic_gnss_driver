...
namespace nmea {
    ...
    private:
        ...
        std::string buffer;
        bool fillingbuffer;
        uint32_t maxbuffersize; // limit the max size if no newline ever comes... Prevents huge
                                // buffer string internally
    ...
    public:
        ...
        void setSentenceHandler(
            std::string cmdKey,
            std::function<void(const NMEASentence&)>
                handler); // one handler called for any named sentence where name is the "cmdKey"
        std::string getRegisteredSentenceHandlersCSV(); // show a list of message names that
        ...
        // This function expects the data to be a single line with an actual sentence in it, else it
        // throws an error.
        void
        readSentence(std::string cmd); // called when parser receives a sentence from the byte
                                       // stream. Can also be called by user to inject sentences.
