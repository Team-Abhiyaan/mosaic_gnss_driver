...
namespace serial_util {
    struct Config
    {
         ...
         * @param flowControl: Flow control - Ability of device to tell another device to stop
         ...
         * Low-latency mode
         * ----------------
         * Serial ports on linux are "wrapped" into unix-style terminal constructs, which hits you
         * with 1 tick lag, i.e. 10ms. On a PC, you can go hardcore and talk to standard serial
         * ports directly, unbind linux driver from serial port hardware and control the port via
         * inb/outb to port registers. The downside is you don't get interrupts when data arrives
         * and you have to poll the register. often. Hence not preferred.
         */
        Config(int32_t baudRate, int32_t dataBits, int32_t stopBits, Parity parity,
    ...
    public:
         ...
         *
         * Appends upto maxBytes into the provided vector. If maxBytes is 0, it reads all available
         * bytes.
         *
         * @param output: The output buffer to which read bytes are to be saved
         * @param maxBytes: The maximum number of bytes to read. If set to 0, all available bytes
         * are read.
         ...
         *
         * @return Number of bytes written on success. On error, -1 is returned, and errno is set to
         ...
         */
        std::string errorMsg() const { return m_ErrorMessage; }
